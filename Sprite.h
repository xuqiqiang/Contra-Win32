...
typedef  int		DRAWTYPE,IMAGETYPE;	//定义类型
...
protected:
	...
	int x;int y;LPCSTR str;int Width;int Height;int Weight;LPCSTR FaceName;COLORREF TextColor;COLORREF BkColor;
...
public:
	...
	void StretchDraw(HDC hDC,int a,int b,int c,int d,
						 int e,int f,int g,int h,BOOL bTrans,COLORREF crTrans);
	void MassDraw(HDC hDC,POINT * pt,int Width,int Height,int len,int type,int nFrameDelay,BOOL bTrans=FALSE,COLORREF crTrans=RGB(0,0,0));
	...
	void Image(HDC hDC,int nRectX,int nRectY,int nRectWidth,int nRectHeight,COLORREF crTrans);

	//绘制半透明位图,不透明度nApha取值为0~255
	void DrawAlpha(HDC hDC,int a,int b,int c,int d,
						 int e,int f,int g,int h,int nAlpha);

	void DrawFont(HDC hDC,int x,int y,LPCSTR str,int Width,int Height,int Weight,LPCSTR FaceName,COLORREF TextColor,COLORREF BkColor);
	...
	void SetDrawAlphaInfo(int a,int b,int c,int d,
						 int e,int f,int g,int h,int nAlpha=255);

	//设置用于Draw()方法的参数
	void SetDrawInfo(int x,int y,BOOL bTrans=FALSE,COLORREF crTrans=RGB(0,0,0));

	void SetStretchDrawInfo(int a,int b,int c,int d,int e,int f,int g,int h,BOOL bTrans=FALSE,COLORREF crTrans=RGB(0,0,0));

	void SetMassDrawInfo(POINT * pt,int Width,int Height,int len,int type,int nFrameDelay=100,BOOL bTrans=FALSE,COLORREF crTrans=RGB(0,0,0));
	...
	//设置用于Rect()方法的参数
	void SetImageInfo(int nRectX,int nRectY,int nRectWidth,int nRectHeight,int it,COLORREF crTrans);
	...
	void SetDrawFontInfo(int x,int y,LPCSTR str,int Width,int Height,int Weight,LPCSTR FaceName,COLORREF TextColor,COLORREF BkColor=RGB(0,0,0));
